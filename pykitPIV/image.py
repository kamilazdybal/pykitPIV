...
class Image:
    ...
    **Attributes:**
    ...
    def log_transform_images(self,
        ...
        :param images_tensor:
            ``numpy.ndarray`` specifying the PIV image pairs tensor. It can be an array of size ``(n_images, image_height, image_width)``
        ...
        try:
            (_, n_pairs, _, _) = np.shape(images_tensor)
            if n_pairs == 2:
                image_pair = True
            else:
                raise ValueError(
                    "Parameter `images_tensor` has to be of size `(n_images, 2, image_height, image_width)` or `(n_images, image_height, image_width)`.")
        except:
            try:
                (_, _, _) = np.shape(images_tensor)
                image_pair = False
            except:
                raise ValueError(
                    "Parameter `images_tensor` has to be of size `(n_images, 2, image_height, image_width)` or `(n_images, image_height, image_width)`.")
        ...
        for i in range(0, images_tensor.shape[0]):
            ...
            if image_pair:
                ...
                log_transformed_images_tensor[i, :, :] = np.log10(images_tensor[i, :, :] + addition)
                log_transformed_images_tensor[i, :, :] = np.log10(images_tensor[i, :, :] + addition)
